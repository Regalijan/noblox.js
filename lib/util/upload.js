...
exports.func = async function (args) {
  ...
  // Asset ID is not included in initial upload response
  // .path is rest of service path to operation

  const operationCheckResponse = await http({
    url: `//apis.roblox.com/assets/user-auth/v1/${path}`,
    method: 'GET',
    resolveWithFullResponse: true,
    jar
  })

  if (operationCheckResponse.statusCode !== 200) { throw new RobloxAPIError(operationCheckResponse) }

  return parseInt(JSON.parse(operationCheckResponse.body).response.assetId)
}
